...
if($cmd=='input' || $cmd=='modify'){
	...
	if($cmd=='modify'){
		...
		VALUES ($seq, '$fnk_uid', '$title', '$link','$type',now(), now(), '');";
...
if($cmd == 'get_contents'){
	...
	$sql = "SELECT seq, fnk_uid, title, link, updt_date, reg_date, comment FROM fav_link where type = '$type' order by seq;";
	// echo $sql;
...
$result = array('RESULT' => 'FAIL','REASON'=>'NO OPTION','test'=>"option : $cmd");
echo json_encode($result);
?>
